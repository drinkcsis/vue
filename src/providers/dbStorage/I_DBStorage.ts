...
export interface I_DBStorage {
    ...
    addPhoto({ albomId, urls, name }: { albomId: number, urls: { origin_url: string, small: string }, name: string }): any
    deletePhoto(photo: PhotoModel): void
    fetchPhotos({ albomId, perPage, page }: { albomId: number, perPage: number, page: number }): any
}